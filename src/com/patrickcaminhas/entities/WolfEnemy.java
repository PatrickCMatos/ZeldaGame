...
public class WolfEnemy extends Entity {
	...
	public static BufferedImage ENEMY_EN = Game.spritesheet.getSprite(80, 80, 16, 16);
	public static BufferedImage ENEMY_FEEDBACK_RIGHT = Game.spritesheet.getSprite(112, 96, 16, 16);
	public static BufferedImage ENEMY_FEEDBACK_LEFT = Game.spritesheet.getSprite(112, 96, 16, 16);
	...
	public WolfEnemy(int x, int y, int WIDTH, int HEIGHT, BufferedImage sprite) {
		super(x, y, WIDTH, HEIGHT, null);
		sprite_right = new BufferedImage[4];
		sprite_left = new BufferedImage[4];

		for (int i = 0; i < 4; i++) {
			sprite_right[i] = Game.spritesheet.getSprite(80, 80 + (i * 16), 16, 16);
			sprite_left[i] = Game.spritesheet.getSprite(96, 80 + (i * 16), 16, 16);
	...
	public boolean isColliding(int xnext, int ynext) {
		...
		for (int i = 0; i < Game.wolfs.size(); i++) {
			WolfEnemy e = Game.wolfs.get(i);
			if (e == this) {
				continue;
			}
	...
	public void render(Graphics g) {
		if (!isDamaged) {
			...
			if (dir == dir_right) {
				g.drawImage(ENEMY_FEEDBACK_RIGHT, this.getX() - Camera.x, this.getY() - Camera.y, null);
			}else if (dir == dir_left) {
				g.drawImage(ENEMY_FEEDBACK_LEFT, this.getX() - Camera.x, this.getY() - Camera.y, null);
			}
